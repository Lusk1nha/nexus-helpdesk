...
O projeto é dividido em _crates_ focados em contextos delimitados:
...
- [x] Construção do `AppState` injetando dependências nos _Use Cases_.

### ⏳ Fase 3: Gateway e Rotas HTTP (Próximo Passo)

- [ ] Criar os _Handlers_ do Axum (receber requisições e mapear para os _Use Cases_).
...
- [x] Gerar _embeddings_ via Ollama (`nomic-embed-text`) e injetar no banco de dados vetorial (Qdrant).
- [x] Implementar busca de contexto semântico (_Retrieval_) para enviar ao `AiWorker` antes de responder o ticket.
