...
export function AboutProject({ variants }: AboutProjectProps) {
  return (
    ...
    >
      {/* Description */}
      <div className="space-y-4 rounded-sm border border-(--border) bg-(--surface) p-4 sm:p-6">
        ...
        <p className="text-sm leading-relaxed text-(--muted)">
          ...
          strict multi-tenant isolation, an event-driven background worker built
          on Tokio, and a React 19 monorepo with shared packages for UI, auth,
          themes and API contracts.
        </p>
      ...
      <div className="rounded-sm border border-(--border) bg-(--surface) p-4 sm:p-6">
        <p className="mb-4 font-mono text-[10px] tracking-widest text-(--muted) uppercase">
          System Architecture
        </p>

        <div className="space-y-1">
          {ARCH_LAYERS.map(
            ({ icon: Icon, label, color, items, connector }, i) => {
