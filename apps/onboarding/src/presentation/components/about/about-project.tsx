  ...
  return (
    <motion.div variants={variants} className="flex flex-col gap-4">
      ...
      <p className="text-sm leading-relaxed text-(--muted)">
        ...
        running Large Language Models (like Llama-3/Phi-3) directly on the
        infrastructure via Ollama.
      </p>
      <p className="text-sm leading-relaxed text-(--muted)">
        Under the hood, it utilizes a blazingly fast{" "}
        <strong className="text-(--fg)">Rust (Axum)</strong> backend, a strict
        Multi-Tenant relational database, and an Event-Driven background worker
        system built with `tokio`. The frontend is a modern React 19 Monorepo
        designed for maximum performance and isolation.
      </p>
    </motion.div>
  )
}
